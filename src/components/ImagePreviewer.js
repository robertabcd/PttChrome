...
export const makeSrcResolver = ({ src }) =>
  resolvers.reduceRight((acc, resolver) => {
    if (!acc && resolver.test(src)) {
      return resolver.request(src);
    }
    return acc;
  }, false);

export const makeImageDOMResolver = ({ src }) =>
...
export class ImagePreviewer extends React.PureComponent {
  ...
  handleStart(props) {
    this.setState((state, { request }) => {
      request.then(this.handleResolve, noop);
      return {
        pending: request,
        value: undefined
      };
...
ImagePreviewer.renderInline = ({ src }) => (
  <img className="easyReadingImg hyperLinkPreview" src={src} />
);
